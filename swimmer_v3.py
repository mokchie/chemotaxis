...
def clear(sname):
    pattern1 = re.compile("^%s-epoch-([0-9]+).data$"%sname)
    pattern2 = re.compile("^%s-actions-epoch-([0-9]+).data$"%sname)
...
class Swimmer:
    def __init__(self, dim=3, v0=1.0, vw=0, k0=1.0, kw=1.0, kn=2, tau0=0, tauw=0, taun=1,t0=0.0, rx0=0.0, ry0=0.0, rz0=0, tx0=1.0, ty0=0.0, tz0=0, nx0=0.0, ny0=1.0, nz0=0, dt=0.002, Taction=1/4, conc_field=Conc_field(), targetx=0.0, targety=10000, targetz=0, lifespan=10, sname='sample', xb=(0,0), yb=(0,0), zb=(0,0), state_size=4, rand=False, dump_freq=1, Regg=1.0, actionAll=True, vel_field=None,xi_noise=0,saving_interval_dt=10):
        ...
        self.action_labels = list(range(len(self.actions)))+[-1,-2] #-1: stay unchanged, -2: tumble
    ...
    def step(self, states, action_label):
        ...
        if action_label>=-1: #-1: stay unchanged, -2: tumble
        ...
        if action_label>=0:
            self.v = self.vs[action_label]
        for i in range(ntimes):
            if self.dim == 2:
                if action_label>=0:
                    self.kappa = self.actions[action_label]
                kappa = self.kappa
                if not self.vel_field:
                ...
                if action_label>=0:
                    self.kappa,self.tau = self.actions[action_label]
                kappa = self.kappa
                tau = self.tau
                if not self.vel_field:
